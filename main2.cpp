...
double get_whole_area(vector<triangle> &triangles, vector<vect> &points){
    double ans = 0.0;
    int ind = 0;
    while(triangles[ind].point1 !=-1){
        ans += get_area(triangles[ind], points);
        ind ++;
    }
    return ans;
}
int main(){
    ...
    for(int i = 0; i < N; i++){
        vect p(cos(i  / double(N) * 2 * 3.14159), sin(i  / double(N) * 2 * 3.14159), 1 * sin(i / double(N) * 2 * 2 * 3.14159));
        contour[i] = p;
    }
    ...
    while(1 - Dl / meanDist > 0){
        ...
        while(point_ind < stop){
            if(point_ind + 1 == stop){
                ...
                pointTriangles[point_ind2].push_back(triangle_ind);
    ...
    int num_of_points = point_ind+10;
    int num_of_triangles = triangle_ind+1;

    //here we do minimisation
    int ITERATIONS = 100;
    out << ITERATIONS << '\n';
    cout << "Initial Size = " << get_whole_area(triangles, points) << '\n';
    for (int iteration = 0; iteration < ITERATIONS; iteration++) {
        for (int node_id = N; node_id < num_of_points; node_id++) {
        ...
        out << '\n';
        cout << "Current Size = " << get_whole_area(triangles, points) << '\n';
    }
